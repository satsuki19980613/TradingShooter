...
export class PhysicsSystem {
  constructor(worldWidth, worldHeight) {
    ...
    this.grid = new SpatialGrid(worldWidth, worldHeight, GameConstants.GRID_CELL_SIZE);
  ...
  update(worldState, dt = 1.0) {
    ...
    activeEntities.forEach(entity => {
        ...
        const nextPos = MovementLogic.updatePosition(entity.x, entity.y, entity.vx, entity.vy);
        entity.x = nextPos.x;
        entity.y = nextPos.y;

        this.resolveObstacleCollisions(entity);
        
        const clamped = CollisionLogic.clampPosition(entity.x, entity.y, entity.radius, this.worldWidth, this.worldHeight);
        entity.x = clamped.x;
        entity.y = clamped.y;
    });

    this.resolveEntityCollisions(activeEntities);
  ...
  resolveObstacleCollisions(entity) {
      ...
      for (const other of nearby) {
          if (other.type === "obstacle_wall") {
              const correctedPos = CollisionLogic.resolveObstacleCollision(entity.x, entity.y, entity.radius, other);
              if (correctedPos) {
  ...
  resolveEntityCollisions(entities) {
      for (let i = 0; i < entities.length; i++) {
          for (let j = i + 1; j < entities.length; j++) {
              ...
              const push = CollisionLogic.resolveEntityOverlap(e1.x, e1.y, e1.radius, e2.x, e2.y, e2.radius);
              if (push) {
                  e1.x += push.pushX;
                  e1.y += push.pushY;
                  e2.x -= push.pushX;
                  ...
                  this.resolveObstacleCollisions(e1);
                  this.resolveObstacleCollisions(e2);
              }
          }
      }
  }
      ...
      for (let i = worldState.bullets.length - 1; i >= 0; i--) {
          ...
          for (const target of nearby) {
              if (target.type === "obstacle_wall") {
                  const collision = CollisionLogic.resolveObstacleCollision(b.x, b.y, b.radius, target);
                  if (collision) {
                      hit = true;
                      break;
                  }
              } else if ((target.type === "player" || target.type === "enemy") && !target.isDead) {
                  if (b.ownerId === target.id) continue;
                  
                  const dist = CollisionLogic.getDistance(b.x, b.y, target.x, target.y);
                  if (dist < b.radius + target.radius) {
                      target.hp -= b.damage;
                      worldState.frameEvents.push({ type: "hit", x: b.x, y: b.y, color: b.type === "enemy" ? "#ff0000" : "#00ffff" });
                      hit = true;
