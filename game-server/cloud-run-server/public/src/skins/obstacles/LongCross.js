...
function drawBaseShape(ctx, isShadow, colors, armSize) {
    ...
    if (!isShadow) {
        ...
        ctx.stroke();
        ctx.beginPath();
        ctx.moveTo(-w, 60); ctx.lineTo(w, 60);
        ctx.moveTo(-w, 140); ctx.lineTo(w, 140);
        ctx.stroke();
    }
}
...
function drawPipeUnit(ctx) {
    ...
    ctx.lineCap = 'butt';
    ctx.beginPath();
    ctx.moveTo(40, -15); ctx.lineTo(-20, -15);
    ctx.moveTo(40, 15); ctx.lineTo(-20, 15);
    ctx.stroke();
    ctx.fillStyle = '#444';
    ctx.beginPath();
...
function drawStorageUnit(ctx, colors) {
    ...
    for(let i=-25; i<30; i+=10) {
            ctx.beginPath();
            ctx.moveTo(i, 15); ctx.lineTo(i+5, 15); ctx.lineTo(i-2, 20); ctx.lineTo(i-7, 20);
            ctx.fill();
    }
}
...
function drawCenterHub(ctx) {
    ...
    ctx.lineWidth = 3;
    ctx.beginPath();
    ctx.moveTo(-35, -35); ctx.lineTo(-20, -20);
    ctx.moveTo(35, -35); ctx.lineTo(20, -20);
    ctx.moveTo(-35, 35); ctx.lineTo(-20, 20);
    ctx.moveTo(35, 35); ctx.lineTo(20, 20);
    ctx.stroke();
...
function drawGlow(ctx, time, colors) {
    ...
    ctx.beginPath();
    const offset = 45;
    ctx.moveTo(-offset, -offset); ctx.lineTo(-20, -20);
    ctx.moveTo(offset, -offset); ctx.lineTo(20, -20);
    ctx.moveTo(-offset, offset); ctx.lineTo(-20, 20);
    ctx.moveTo(offset, offset); ctx.lineTo(20, 20);
    ctx.stroke();
    ...
    ctx.beginPath();
    ctx.moveTo(0, 50); ctx.lineTo(0, 150);
    ctx.stroke();
}