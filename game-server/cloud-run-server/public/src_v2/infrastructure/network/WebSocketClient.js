...
export class WebSocketClient {
  ...
  handleBinaryMessage(buffer) {
    ...
    if (msgType === 1) { // MSG_TYPE_DELTA
        ...
        for(let i=0; i<numP; i++) {
            const p = {
                i: reader.string(), x: reader.f32(), y: reader.f32(),
                h: reader.u8(), a: reader.f32(), n: reader.string(),
                lastAck: reader.u32(), d: reader.u8(), e: reader.u16(), ba: reader.u16()
            };
            const hasCharge = reader.u8();
            if(hasCharge) {
                p.cp = { ep: reader.f32(), a: reader.f32(), t: reader.u8()===1 ? "short" : "long" };
            }
        ...
        for(let i=0; i<numB; i++) {
            const b = { i: reader.string(), x: reader.f32(), y: reader.f32(), a: reader.f32() };
            const tId = reader.u8();
            const types = ["player", "enemy", "player_special_1", "item_ep", "player_special_2", "player_special_3", "player_special_4"];
        ...
        for(let i=0; i<numEv; i++) {
            ...
            delta.events.push({
                ...
                x: reader.f32(), y: reader.f32(), color: reader.string()
            });
        }
  ...
  sendInput(seq, states, pressed, mousePos) {
    ...
    view.setUint8(0, 2); // MSG_TYPE_INPUT
